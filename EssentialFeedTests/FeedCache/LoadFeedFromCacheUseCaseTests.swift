...
final class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deletesCacheOnRetrievalEror() throws {
        let (sut, store) = makeSUT()
        sut.load { _ in }
    ...
    func test_load_doesNotDeleteCacheOnEmptyCache() throws {
        let (sut, store) = makeSUT()
        sut.load { _ in }
        store.completeRetrievalWithEmptyCache()
        XCTAssertEqual(store.receivedMessages, [.retrieve])
    ...
    func test_load_deletesCacheOnSevenDaysOldCache() throws {
        let feed = uniqueFeed()
        let fixedCurrentDate = Date.now
        let sevenDaysOldTimestamp = fixedCurrentDate.adding(days: -7)
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
    ...
    func test_load_deletesCacheOnMoreThanSevenDaysOldCache() throws {
        let feed = uniqueFeed()
        let fixedCurrentDate = Date.now
        let sevenDaysOldTimestamp = fixedCurrentDate.adding(days: -7).adding(seconds: -1)
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
